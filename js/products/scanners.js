...
const data = [
    ...
    {
        link: "scanner_stand_universal.php",
...
function filterArray() {
    let updatedArray = initialArray
        ...
        .filter(function (a) {
            if (filterParam.drain.length !== 0) {
                for (let i = 0; i < filterParam.drain.length; i++) {
                    if (a.drain === filterParam.drain[i]) {
                        return a;
                    }
                }
            } else {
...
const drain1 = document.getElementById('drain1');
const drain2 = document.getElementById('drain2');
...
function updateFilterParam() {
    ...
    if (drain1.checked === true) {
        filterParam.drain.push(true);
    }

    if (drain2.checked === true) {
        filterParam.drain.push(false);
    }
...
function drain1Click() {
    if (drain1.checked == true) {
        filterOptionArray.push('Стойки для сканера')
        clickArr.push(drain1)
        arrCounter = true;
    } else {
        spliceMethod(drain1);
        spliceMethodSecond('Стойки для сканера')
        arrCounter = false;
    }
    updateFilterParam()
}

function drain2Click() {
    if (drain2.checked == true) {
        filterOptionArray.push('Сканеры')
        clickArr.push(drain2)
        arrCounter = true;
    } else {
        spliceMethod(drain2);
        spliceMethodSecond('Сканеры')
        arrCounter = false;
    }
    updateFilterParam()
}
...
drain1.addEventListener('click', drain1Click)
drain2.addEventListener('click', drain2Click)
...
function countParam(array) {
    ...
    for (let index = 0; index < array.length; index++) {
        ...
        if (array[index].drain == true) {
            drain1++;
        } else if (array[index].drain == false) {
            drain2++;
        }
    ...
    return [scanType1, scanType2, scanType3, screenScan1, screenScan2, connections1, connections2, connections3, readType1, readType2, readType3, drain1, drain2,]
